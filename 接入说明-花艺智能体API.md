...
> **版本**：v3（2026-09，平台只读重构 + 店铺锁定 + AI 贺卡后）。本文档回答一个问题：**把「跳舞兰」花卉智能体接进你的业务，到底要做哪些事。**
...
|---|---|---|
| `plan` | 可售花束 / 方案 / 商品 | id、名称、价格、描述、图片、标签或分类、**所属店铺 id**（要用「店铺锁定」时必填） |
| `shop` | 可配送店铺 | id、名称、地址、评分、配送信息 |
| `order` | 订单（查单用） | id、用户id、方案id、金额、状态、创建时间、**所属店铺 id**（要用「店铺锁定」时必填） |
| `user` | 用户（可选） | id、昵称等 |

> **店铺列很关键**：`plan` / `order` 映射里如果有「所属店铺」这一列（映射到标准字段 `shop_id`），智能体就能在 SQL 层把结果**硬限定在指定店铺**内。没有这一列时不会报错，但智能体只能拿到全量数据后由模型自行筛选——效率和准确度都会打折。要用下面的「店铺锁定模式」，**强烈建议把这一列映射上**。
...
### 4.6 店铺锁定模式（用户从具体店铺进入，推荐）

用户在**某家店铺的页面**里唤起智能体时，把该店铺 ID 通过 `shop_id` 一并传进来，整轮会话就锁定在这家店：
...
POST /chat
{
  "message": "给我妈挑束花",
  "user_id": "wxmini_xxx",
  "shop_id": "S001"        // ← 该店铺在平台库里的真实 ID
}
```

锁定后智能体自动遵守以下行为（前端无需额外处理）：

| 维度 | 锁定后的行为 |
|---|---|
| 选店环节 | **完全跳过**：不再查店铺列表、不再推 `shop_card`、不再问「去哪家店」 |
| 商品 / 方案 | 查询自动限定该店（SQL 层按 `shop_id` 过滤）；映射缺店铺列时由模型按行内 `shop_id` 字段自行筛选 |
| DIY 定制 | 主花 / 配材 / 叶材 / 包装**只选用该店在售**的花材与商品；不确定在售清单时会先查再设计 |
| 下单 | 直接以该店铺提交 `create_order`；**传其他店铺、或方案归属其他店铺的请求会被拒绝并报错**（不会误下单到别家） |

**前端要做的事**：锁定模式下不会返回 `shop_card`，所以店铺卡片不是必经组件（但非锁定场景仍会出现，建议实现）。同一 `session_id` 会记住锁定关系，后续轮次不必重复传 `shop_id`（重传也可以，但需与首次一致）。

**平台方要做的事**：把 `plan` / `order` 的「所属店铺」列映射到标准字段 `shop_id`（见 §A2 备注），否则 SQL 层无法过滤。

**边界行为**：该店确实没有用户想要的花材时，智能体会如实说明并给出这家店能做的替代方案，**不会拿别家商品来凑**。
...
- [ ] `plan` / `order` 已映射「所属店铺」列到 `shop_id`（要用店铺锁定模式时必填）
- [ ] 店铺锁定场景：传 `shop_id` 后返回结果全部属于该店铺
...
- [ ] **店铺锁定模式**：从店铺页进入时传了 `shop_id`，全程没有出现「选哪家店」的追问，也没有返回 `shop_card`
- [ ] **店铺锁定模式**：推荐的商品、DIY 方案的原料都属于该店铺（拿别店商品对比验证一次）
