...
from agent.toolkit import allowed_entities, register_tool
...
@register_tool(name='platform_db_test_connection', description='测试目标平台外部数据库连接和服务端身份，仅执行只读探测，不会写入数据。', parameters={'type': 'object', 'properties': {'source_id': {'type': 'string', 'description': '外部数据源 ID，对应 PLATFORM_DB_<SOURCE_ID>_URL 环境变量'}}, 'required': ['source_id']}, tags=['database', 'discovery', 'external', 'ops'])
...
@register_tool(name='platform_db_sample_table', description='从目标平台外部数据库的指定合法表读取最多 5 行脱敏样本，不接受任意 SQL。', parameters={'type': 'object', 'properties': {'source_id': {'type': 'string'}, 'schema': {'type': 'string'}, 'table': {'type': 'string'}, 'limit': {'type': 'integer'}}, 'required': ['source_id', 'schema', 'table']}, tags=['database', 'discovery', 'external', 'ops'])
...
def platform_db_query_entity(source_id: str, entity: str, keyword: str='', limit: int=100, shop_id: str='', id: str='', _context: dict | None=None) -> str:
    ...
    _allowed = allowed_entities()
    if _allowed and str(entity or '').strip().lower() not in _allowed:
        return tool_result(False, error='当前实例不提供该实体的查询，请在方案设计与养护建议的范围内为用户解答')
...
@register_tool(name='platform_mapping_draft', description='根据 platform_db_discover 返回的脱敏 schema profile 生成结构化映射草案，包含候选表字段、证据、置信度和风险；仅 draft，不会写文件、激活映射或写入目标库。', parameters={'type': 'object', 'properties': {'profile': {'type': 'object'}}, 'required': ['profile']}, tags=['database', 'mapping', 'external', 'ops'])
...
@register_tool(name='platform_mapping_save_draft', description='将已生成的外部平台映射草案保存到智能体内部 PostgreSQL，自动生成版本并写入审计；不会激活映射或写入目标平台。', parameters={'type': 'object', 'properties': {'profile': {'type': 'object'}, 'draft': {'type': 'object'}, 'actor': {'type': 'string'}}, 'required': ['profile', 'draft']}, tags=['database', 'mapping', 'audit', 'ops'])
...
@register_tool(name='platform_mapping_list_drafts', description='读取指定外部平台数据源的映射草案版本列表，仅返回元数据，不返回敏感连接信息。', parameters={'type': 'object', 'properties': {'source_id': {'type': 'string'}, 'limit': {'type': 'integer'}}, 'required': ['source_id']}, tags=['database', 'mapping', 'audit', 'ops'])
...
@register_tool(name='platform_mapping_set_status', description='更新映射草案审核状态：reviewed/approved/active/revoked；状态变更写入内部审计，active 时同数据源旧版本自动撤销。', parameters={'type': 'object', 'properties': {'mapping_id': {'type': 'string'}, 'status': {'type': 'string'}, 'actor': {'type': 'string'}}, 'required': ['mapping_id', 'status', 'actor']}, tags=['database', 'mapping', 'audit', 'ops'])
...
@register_tool(name='platform_mapping_get_active', description='读取指定外部数据源当前 active 映射；没有明确激活版本时返回空，不会回退到草案。', parameters={'type': 'object', 'properties': {'source_id': {'type': 'string'}}, 'required': ['source_id']}, tags=['database', 'mapping', 'ops'])
