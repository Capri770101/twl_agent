...
class ReActAgent:
    ...
    def _build_system(self, stage: SessionStage, long_term: dict[str, str], shop_id: str | None=None) -> str:
        ...
        parts = [
            ...
            '     - business_hours 营业时段（如 07:00-22:00）',
            '     - status 营业状态（如 open / closed）',
            '     - delivery_time 配送时长（如 30-60分钟）',
            '     - delivery_fee 配送费、min_order_price 起送价（已是元，不要再做分转元）',
            '     - phone 电话、address 地址、rating 评分',
            '  3. 判断「现在是否营业」：把 business_hours 的时段与上文「当前时间」比对，落在区间内即营业中；',
            '     status 是平台库的静态值，可能与时段推算不一致——两者冲突时以时段推算为准，并可补一句「具体以店铺实际为准」。',
            '  4. 用 respond_to_user 结束：给店铺卡 ui="shop_card" data={shops:[...]}，或纯文字简答 ui="text"',
            '  注意：这些是平台库的静态配置而非实时计算；查不到就如实说查不到，绝不编造营业时间或配送承诺。',
            '',
            '### 场景7：平台数据未接入 / 查询报错',
